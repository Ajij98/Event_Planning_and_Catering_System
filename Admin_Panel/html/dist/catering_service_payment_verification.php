<?php
 ...
 ?>
  ...
  <!--Catering Service Payment Activation Section-->
  <?php
   $db = new Database();
     ...
     if(isset($_GET['payment_id']))
     {
       ...
       $sql = "SELECT is_accepted FROM tb_caterer_payment WHERE is_accepted = 0 AND payment_id = $payment_id LIMIT 1";

       $result = $db->link->query($sql) or die($this->link->error.__LINE__);

       if($result->num_rows == 1)
       {
         $sql = "UPDATE tb_caterer_payment SET is_accepted = 1 WHERE payment_id = $payment_id LIMIT 1";
