<?php
 ...
 ?>
  ...
  <!-- Caterer Payment Table Data Load -->
 <?php
   $user_name       = $_SESSION['user_name'];
   $db              = new Database();
   $sql             = "SELECT * FROM tb_caterer_payment";
   $read_caterer_payment = $db->select($sql);
  ?>
...
<head>
    ...
    <title>Divine Planners - Catering Service Payment | Admin Panel</title>
        ...
        <nav class="page-sidebar" id="sidebar">
            <div id="sidebar-collapse">
                ...
                <ul class="side-menu metismenu">
                    ...
                    <li>
                        <a class="active" href="catering_service_payment.php"><i class="sidebar-item-icon fa fa-money"></i>
                    ...
                    <div class="ibox-body">
                        <table class="table table-striped table-bordered table-hover" id="example-table" cellspacing="0" width="100%">
                            ...
                            <tbody>
                                ...
                                <?php if($read_caterer_payment){ $i = 0; ?>
                                <?php while($result = $read_caterer_payment->fetch_assoc()){ $i = $i + 1; ?>
                                <tr>
                                    <td class="text-primary font-weight-bold"><?php echo "Payment Id-".$result['payment_id']; ?></td>
                                    <td><?php echo $result['payment_code']; ?></td>
                                    <td><?php echo "Company Id-".$result['caterer_id']; ?></td>
                                    ...
                                    <td>
                                        <?php
                                            ...
                                            {
                                            ?>
                                                <a href="catering_service_payment_verification.php?payment_id=<?php echo $result['payment_id']; ?>" onclick="return confirm('Sure to accept payment?')" class="btn btn-success btn-rounded btn-sm" >Accept Payment</a>
                                            <?php
                                            }
                                        ?>
                                    </td>
                                    <td>
                                        <a href="delete_data.php?caterer_payment_id=<?php echo $result['payment_id']; ?>" title="delete" onclick="return confirm('Sure to delete?')" class="btn btn-danger btn-sm px-2 py-1" style="border-radius: 3px;"><i class="fa fa-trash-o"></i></a>
                                    </td>
                                </tr>
                                <?php } ?>
                                <?php }else{ ?>
